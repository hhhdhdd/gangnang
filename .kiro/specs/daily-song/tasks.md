...
| I | Suno multi-key: пул ключей в меню, авто-проверка кредитов, ротация при <10 / 0 | `[~]` | 0 | 4 | 4 |
...
## Фаза I — Suno multi-key + ротация по кредитам

> **Все задачи ниже — в открытом PR (`feat/suno-multi-key`).** После мерджа `[~]` → `[x]`.

Запрошено владельцем:
> «Добавить несколько API-ключей Suno, чтобы ротировались, когда кончаются
> фри-кредиты; авто-проверять остаток и заранее менять ключ, если меньше 10.»

- [~] **I.1** Таблица `suno_keys` (миграция 0010) + модель `SunoKey`: пул ключей, `is_active` / `enabled` / `last_credits` / `last_checked_at`. Уникальность по `api_key`.
- [~] **I.2** `app/services/suno.py`: CRUD (`add_key`/`list_keys`/`remove_key`), выбор активного (`get_active_key` с авто-избранием), ротация (`rotate_active` — лучший по кредитам, unknown=высокий приоритет), `ensure_usable_key` (проактивно по кэшу при <10), `set_key_credits` (0 → выключить), `refresh_all_credits`, `generate_with_rotation` (реактивно: на 429 выключает ключ и пробует следующий). Легаси `suno.api_key` авто-мигрируется в пул. `get/set/clear_api_key` теперь поверх пула.
- [~] **I.3** Scheduler-job `suno:credits_refresh` (каждые 30 мин): обновляет баланс всех ключей, выключает пустые, проактивно ротирует активный, если <10.
- [~] **I.4** UI `/suno → 🔑 Ключи (N)`: список с маркерами (✅ активный / ▫️ резерв / 🔴 выключен) + кредиты, «➕ Добавить», «🗑» на ключ, «🔄 Обновить кредиты». Генерация (`/music`, песня дня, тест-ген) ходит через `generate_with_rotation` / `ensure_usable_key`.

**Definition of done фазы I**:
1. `/suno → 🔑 Ключи` → добавить 2+ ключа, видеть кредиты и активный.
2. Когда у активного <10 кредитов — на следующей проверке/генерации бот сам переключается на резерв.
3. Если ключ вернул 429 (кончились) в момент submit — бот выключает его и пробует следующий, песня всё равно генерится.
